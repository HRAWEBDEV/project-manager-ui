"use client";
import { type OrganizationMember } from "../../../organizations/services/organizationsApiActions";
...
export default function AddWorkspaceMemberItem({
  member,
  onAddSuccess,
}: {
  member: OrganizationMember;
  onAddSuccess?: () => unknown;
}) {
  const confirmAddMember = useAddWorkspaceMember();
  const {
    shareDictionary: {
  ...
  return (
    <div key={member.id} className="relative">
      <div className="absolute top-2 inset-e-1">
        <Button
          disabled={confirmAddMember.isPending}
          onClick={() => {
            confirmAddMember
              .mutateAsync({
        ...
        >
          {confirmAddMember.isPending ? <Spinner /> : <BsPersonFillAdd />}
        </Button>
      </div>
      <Button
        variant="outline"
