...
- Renders a dedicated scoped Axios interceptor component (`OrganizationAxiosInterceptor`, `WorkspaceAxiosInterceptor`) that, in a `useEffect` keyed on the active entity, registers a request interceptor adding an `organization-id`/`workspace-id` header and ejects it on cleanup/change.
- Also fires a `useOrganizationPermissions`/`useWorkspacePermissions` query (`organizations/hooks/useOrganizations.ts`, `[organization]/workspaces/hooks/useWorkspaces.ts`; endpoints `${xBaseApi}/permissions`), `enabled: !!activeX`, `staleTime: "static"`. This is a cache-warming fetch only — the query result isn't threaded into the context value; consumers elsewhere call the same `useXPermissions()` hook and read it off the shared TanStack Query cache rather than pulling it from `useOrganizationContext`/`useWorkspacesContext`.
...
- **`[workspace]/services/settings/`** — the settings modal, mounted in `[workspace]/layout.tsx` (not `(panel)/layout.tsx`, since it renders workspace-scoped tabs like the active workspace's edit form). `SettingsProvider` owns `{ open, activeTab, showConfirmLogout }` and registers the `toggleSettings`/`toggleShortcuts` hotkeys itself (via `useShortcutsContext()`) to open the modal on a given tab. `utils/settingItems.ts` is the ordered tab list (`SettingTab` union) and `utils/getSettingsIcon.tsx` maps each tab key to an icon; `components/SettingsModal.tsx` renders the tab list from `settingItems` and switches on `activeTab` to delegate content to each domain's *existing* component (`UserWrapper`, `OrganizationWrapper`, `EditWorkspace`, `WorkspacesWrapper`, `ShortcutsWrapper`, `UserInterfaceWrapper` for the `userInterface` tab) rather than duplicating that UI — add a settings tab by adding a `settingItems` entry, an icon case, and a `renderSettingContent` case pointing at the feature's own component. `UserInterfaceWrapper` itself lives outside the settings folder, at `(panel)/services/userInterface/components/` (a plain component, not a Context/Provider feature — it just reads/writes `useBaseConfig()` and `next-themes`' `useTheme()` directly), since theme/color state is panel-wide rather than settings-owned.
- **`[workspace]/services/history/`** — `HistoryProvider` tracks a `redirectCount` (bumped on every `usePathname()` change) and an `activePath` (the workspace-relative path segment, `pathname.split("/")[4] || "home"`), both exposed via `useHistoryContext()`. `components/HistoryControllers.tsx` is the back-navigation button that reads `redirectCount`; `[workspace]/components/tabs/TabsNav.tsx` (the mobile bottom tab bar) reads `activePath` to drive the active `Tabs` value against `utils/navigationItems.tsx`. Despite living under `[workspace]/`, it's mounted at `(panel)/layout.tsx` via a relative import, so it's available panel-wide, not just within a workspace.
...
`services/base-config/BaseConfigProvider.tsx` owns locale switching (`setLocale` rewrites the URL's locale segment and does a full navigation), wraps `next-themes`' `ThemeProvider` (`light`/`dark`/`system`, read/set via `next-themes`' own `useTheme()` rather than a context field), and owns the color-palette switcher: `onChangeColorTemplate` toggles a class from `app/utils/appTemplates.ts`'s `appColorTemplates` union onto `document.documentElement`, persists the choice via `utils/colorPalleteManager.ts` (`localStorage` key `active-color-pallete`), and re-applies the saved class on mount. Both theme mode and color palette are edited from `UserInterfaceWrapper` (the settings modal's `userInterface` tab, see above).
...
`app/utils/defaultAxios.ts` exports a shared axios instance (`baseURL` = `NEXT_PUBLIC_API_URI`). `app/[lang]/services/axios-interceptors/AxiosBaseConfig.tsx` is a client component (mounted once in the root layout) that registers a request interceptor adding `languageID` and `apptype` headers, kept in sync with the active locale via `useBaseConfig()`. Deeper layers layer their own scoped interceptors on the *same* shared instance (see `OrganizationAxiosInterceptor`/`WorkspaceAxiosInterceptor` above) rather than creating a new axios instance per feature — all API calls (e.g. `authApiActions.ts`, `*ApiActions.ts`) import the one shared `axios` instance from `app/utils/defaultAxios.ts`.
